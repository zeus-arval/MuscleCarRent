...
namespace Infra.Common
{
     public abstract class BaseRepo<T>
        ...
        protected internal T get(string id) {
            ...
            var o = dbSet?.AsNoTracking().FirstOrDefault(m => ((IEntityData) m).Id == id);
            return o;
        }
        ...
        protected internal async Task<T> getAsync(string id) {
            ...
            var o = await dbSet.AsNoTracking().FirstOrDefaultAsync(m => ((IEntityData) m).Id == id);
            return o;
        }
        protected internal async Task<bool> delete(T obj) {
            var o = await dbSet.FindAsync(((IEntityData) obj).Id);
        ...
        private async Task<bool> isEntityOk(T obj, bool isNewItem) {
            ...
            EntityInDb = await getAsync(((IEntityData) obj).Id);
            return (EntityInDb is null) == isNewItem
                   || errorMessage(
                       isNewItem
                           ? $"Item with id = <{((IEntityData) obj).Id}> already in database"
                           : $"No item with id = <{((IEntityData) obj).Id}> in database");
        }
